...
def text(min_words=15, max_words=200):
    return " ".join([unicode(int(random.random()*100000)) for x in range(0,int(random.random()*max_words+min_words))])
...
def string(max_length=255, min_length=5):
    return chomsky()[:int(random.random()*(max_length-min_length)+min_length)].replace('\n', ' ').replace('\r', '')
    
def datetime(interval=182):
    import datetime
    return datetime.datetime.today()+datetime.timedelta(int(round(random.random()*interval-interval/2)))

def date(interval=182):
    import datetime
    return datetime.date.today()+datetime.timedelta(int(round(random.random()*interval-interval/2)))

def oneof(choices):
    return choices[int(round(random.random()*(len(choices)-1)))][0]

def integer(min=0, max=50):
    return int(round(random.random()*(max-min)+min))

def float(min=0, max=50):
    return random.random()*(max-min)+min

def text(max_length=10, min_length=1):
    return chomsky(integer(min_length, max_length))
